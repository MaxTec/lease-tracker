...
const TenantDashboard: React.FC<TenantDashboardProps> = ({
  ...
  const [editingPhoneLeaseId, setEditingPhoneLeaseId] = useState<number | null>(
    null
  );
          ...
          <Descriptions
            ...
            items={[
              ...
              { label: tLease("endDate"), children: formatDate(lease.endDate) },
              {
                label: tLease("status"),
                children: (
                  <Badge status={getBadgeStatus(lease.status)}>
                    {lease.status}
                  </Badge>
                ),
              },
                ...
                render: (payment) => (
                  <div className="flex items-center gap-2">
                    ...
                    {payment.status === "OVERDUE" && (
                      <Badge status="error" aria-label="Overdue Payment">OVERDUE</Badge>
                    )}
                  </div>
                ),
              },
