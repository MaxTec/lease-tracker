...
type NestedObject = {
  [key: string]: string | number | boolean | null | undefined | NestedObject | NestedObject[];
};

type NestedValue = string | number | boolean | null | undefined | NestedObject | NestedObject[];
  ...
  const sortedData = useMemo(() => {
    ...
    if (sortConfig !== null) {
      sortableData.sort((a, b) => {
        const aValue = getNestedValue(a as unknown as NestedObject, sortConfig.key);
        const bValue = getNestedValue(b as unknown as NestedObject, sortConfig.key);
              ...
              <tr key={item.id} className="hover:bg-gray-50">
                {columns.map((column) => (
                  <td key={column.key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {column.render
                      ? column.render(item)
                      : String(getNestedValue(item as unknown as NestedObject, column.key) ?? '')}
                  </td>
                ))}
              </tr>
            ))}
